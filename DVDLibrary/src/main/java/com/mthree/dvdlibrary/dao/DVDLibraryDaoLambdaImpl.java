...
public class DVDLibraryDaoLambdaImpl implements DVDLibraryDao {
    ...
    public List<DVD> getMoviesReleasedInYear(int year) throws DVDLibraryDaoException {
            return getAllDVDs().stream()
                    .filter((m) -> (m.getReleaseDate().equalsIgnoreCase(Integer.toString(year))))
                    .collect(Collectors.toList());
    }

    public List<DVD> getMoviesWithRating(String rating) throws DVDLibraryDaoException {
        return getAllDVDs().stream()
                .filter((m) -> (m.getRating().equalsIgnoreCase(rating)))
                .collect(Collectors.toList());
    }

    public Map<String,List<DVD>> getMoviesWithDirector(String director) throws DVDLibraryDaoException {
        return getAllDVDs().stream()
